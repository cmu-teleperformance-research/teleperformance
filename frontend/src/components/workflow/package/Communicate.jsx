...
export default function PackageCommunicate({ onAdvance, workflow, workflowData, updateData }) {
  const sc = workflow.screens.communicate;
  const { resolutionRecord } = workflow;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-green-200 bg-green-50 rounded-xl p-4 text-sm space-y-1">
        <div><span className="text-gray-500">Replacement Order Number: </span><strong>{resolutionRecord.replacementOrderNumber}</strong></div>
        <div><span className="text-gray-500">Investigation Case Number: </span><strong>{resolutionRecord.investigationCaseNumber}</strong></div>
      </div>
